...
using YotsubaEngine.YTBContentBuilder.GameData;
...
AudioRegistryGenerator.GenerateRegistry(assetsDirectory, Path.Combine(platformsCoreDirectory, "AutoGeneratedHelper/AudioRegistry"));
AssetRegistryGenerator.GenerateRegistry(assetsDirectory, Path.Combine(platformsCoreDirectory, "AutoGeneratedHelper/AssetRegistry"));
ModelRegistryGenerator.GenerateRegistry(assetsDirectory, Path.Combine(platformsCoreDirectory, "AutoGeneratedHelper/ModelRegistry"));
SpriteSheetPathValidator.ValidateImagePaths(assetsDirectory);
GameDataGenerator.GenerateRegistry(assetsDirectory, Path.Combine(platformsCoreDirectory, "AutoGeneratedHelper/GameDataRegistry"));
//SpriteSheetFiles.CheckSpriteSheetFiles(assetsDirectory, $"Assets");

// Generar YTBFileToGameData.Generated.cs a partir de los atributos [UIComponent]/[UIComponentValue]
// del engine. Esto produce código estático (sin reflexión en runtime) compatible con AOT.
string engineSourcePath = Path.GetFullPath(Path.Combine(currentDirectory, "..", "..", "..", "..", "..", "YotsubaEngine"));
if (!Directory.Exists(engineSourcePath))
    engineSourcePath = Path.GetFullPath(Path.Combine(Directory.GetCurrentDirectory(), "..", "..", "YotsubaEngine"));
if (Directory.Exists(engineSourcePath))
{
    string ytbGeneratedPath = Path.Combine(engineSourcePath, "ActionFiles", "YTB Files", "YTBFileToGameData.Generated.cs");
    YotsubaEngine.YTBContentBuilder.GameData.YTBFileToGameDataGenerator.Generate(engineSourcePath, ytbGeneratedPath);
}
else
{
    Console.WriteLine("[Builder] WARNING: No se encontró el directorio del engine para regenerar YTBFileToGameData.Generated.cs");
}
