...
const COMPONENT_TEMPLATES = {
    ...
    // Componente de c�mara 3D basado en EntityYTBXmlTemplate.CameraTemplate
    CameraComponent3D: [
    ...
    for (const fileName of defaultYtbCandidates) {
        ...
        for (const candidatePath of candidatePaths) {
            try {
                ...
                renderSceneList();
                showToast(`✨ ${fileName} cargado automáticamente`);
...
function deleteCurrentScene() {
    ...
    showConfirmModal(
        '�Eliminar escena?',
        `�Est�s seguro de que deseas eliminar la escena "${gameData.scene[currentSceneIndex].name}"? Esta acci�n no se puede deshacer.`,
        () => {
...
function deleteEntity(entityIndex) {
    showConfirmModal(
        '�Eliminar entidad?',
        `�Est�s seguro de que deseas eliminar esta entidad? Esta acci�n no se puede deshacer.`,
        () => {
