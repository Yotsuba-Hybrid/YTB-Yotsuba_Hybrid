...
namespace YotsubaEngine.ActionFiles.YTB_Files
{
    ...
    /// La parte de parseo "genérica" se autogenera en <c>YTBFileToGameData.Generated.cs</c> por YTBContentBuilder
    /// a partir de los atributos [UIComponent]/[UIComponentValue]. Las implementaciones específicas (custom)
    /// llaman al método <c>Parse{Component}_Generated</c> al inicio y luego sobrescriben los casos especiales.
    /// </summary>
    public partial class YTBFileToGameData
    {
        ...
        public async static Task UpdateStateOfSceneManager()
        {
            ...
            // Restaurar escena previa o usar "Index"
            string targetScene = !String.IsNullOrEmpty(YTBGlobalState.LastSceneNameBeforeUpdate)
                ? YTBGlobalState.LastSceneNameBeforeUpdate : "Index";

            if (sceneManager.Scenes.Count > 0)
            {
                sceneManager.CurrentScene = sceneManager.Scenes._ytb.Any(x => x.SceneName == targetScene)
                    ? sceneManager.Scenes._ytb.FirstOrDefault(x => x.SceneName == targetScene)
        ...
        /// <summary>
        /// Construye las escenas y entidades dentro de un SceneManager a partir de los datos YTB.
        /// <para>Builds scenes and entities into a SceneManager from YTB data.</para>
        /// </summary>
        private static void BuildSceneManager(SceneManager sceneManager, YTBGameInfo gameInfo)
        {
            foreach (var element in gameInfo.Scene)
            {
                    ...
                    foreach (var component in en.Components)
                    {
                        ...
                        switch (component.ComponentName)
                        {
                            ...
                            case nameof(ShaderComponent):
                                ...
                                break;
                            default:
                                // Fallback: componentes nuevos descubiertos por YTBContentBuilder y registrados en
                                // _autoGeneratedComponents (definido en YTBFileToGameData.Generated.cs).
                                if (_autoGeneratedComponents != null
                                    && _autoGeneratedComponents.TryGetValue(component.ComponentName, out var autoAdd))
                                {
                                    autoAdd(entity, component, scene, element.Name, entity.Name);
                                    break;
                                }
#if YTB
        ...
        private static AnimationComponent2D ConvertToAnimation(YTBComponents component, string name, string sceneName = "")
        {
            ...
            {
                animationComponent2D.CurrentAnimationType = currentAnimType;
                animationComponent2D.CurrentAnimation = (currentAnimType, animationComponent2D.GetAnimation(currentAnimType));
            }
#if YTB
