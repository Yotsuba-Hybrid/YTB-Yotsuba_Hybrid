...
private static bool _G_ShouldSkip(string[]? exclude, string name)
{
 ...
 EntityManager entityManager, YTBComponents comp, string sceneName, string entityName, string[]? exclude = null)
 {
 Vector3 _initialPosition = Vector3.Zero;
 float _angleView = 45f;
 float _nearRender = 0.1f;
 float _farRender = 1000f;
 string _entityName = "";
 Vector3 _offsetCamera = new Vector3(0, 50, -100);
 foreach (var prop in comp.Propiedades)
 {
 if (_G_ShouldSkip(exclude, prop.Item1)) continue;
 switch (prop.Item1)
 ...
 case "EntityName":
 {
 _entityName = prop.Item2;
 break;
 }
 case "InitialPosition":
 {
 if (_G_TryParseVector3(prop.Item2, out var v)) _initialPosition = v;
 break;
 }
 case "OffsetCamera":
 {
 if (_G_TryParseVector3(prop.Item2, out var v)) _offsetCamera = v;
 break;
 }
 case "AngleView":
 {
 if (float.TryParse(prop.Item2, NumberStyles.Float, CultureInfo.InvariantCulture, out var v)) _angleView = v;
 break;
 }
 case "NearRender":
 {
 if (float.TryParse(prop.Item2, NumberStyles.Float, CultureInfo.InvariantCulture, out var v)) _nearRender = v;
 break;
 }
 case "FarRender":
 {
 if (float.TryParse(prop.Item2, NumberStyles.Float, CultureInfo.InvariantCulture, out var v)) _farRender = v;
 break;
 }
 }
 }
 var result = new CameraComponent3D(entityManager, _initialPosition, _angleView, _nearRender, _farRender);
 result.EntityName = _entityName;
 result.OffsetCamera = _offsetCamera;
 return result;
 }
